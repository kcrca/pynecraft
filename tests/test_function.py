...
class TestFunctions(unittest.TestCase):
    ...
    def test_as_format(self):
        self.assertEqual(None, as_version(None))
        self.assertEqual([13], as_version([13]))
        self.assertEqual([13], as_version(13))
        self.assertEqual([13], as_version(13.0))
        self.assertEqual([13], as_version('13'))
        self.assertEqual([13], as_version((13,)))
        self.assertEqual([13, 5], as_version([13, 5]))
        self.assertEqual([13, 5], as_version((13, 5)))
        self.assertEqual([13, 5], as_version(13.5))
        self.assertEqual([13, 5], as_version('13.5'))
        self.assertEqual([13, 5], as_version([13, 5, 0]))
        self.assertEqual([13, 5], as_version([13, 5, 0, 0, 0]))
        self.assertEqual([13, 5], as_version("13.5.0"))
        with self.assertRaises(ValueError):
            as_version(())
        with self.assertRaises(ValueError):
    ...
    def test_compare_format(self):
        self.assertTrue(version_cmp(None, None) == 0)
        self.assertTrue(version_cmp(1, None) < 0)
        self.assertTrue(version_cmp(None, 1) > 0)
        self.assertTrue(version_cmp('1.2.3', '1.2.3') == 0)
        self.assertTrue(version_cmp('1.2.3', '1.2.3.4') > 0)
        self.assertTrue(version_cmp('1.22', '1.1') < 0)
        self.assertTrue(version_cmp('1.22.9.9', '1.23') > 0)
