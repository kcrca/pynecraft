...
# TeamOptions
# Derived from https://minecraft.wiki/Commands/team, 2025-01-02T17:56:53-08:00
__teamoption_dups = {}
...
team_options = {
    "DISPLAY_NAME": TeamOptionInfo("""display Name""", "displayName", """Sets the display name of the team.""", "JsonDef"),
    "COLOR": TeamOptionInfo("""color""", "color", """Decides the color of the team and players in chat, above their head, on the Tab menu, and on the sidebar. Also changes the color of the outline of the entities caused by the Glowing effect.""", "JsonDef"),
    "FRIENDLY_FIRE": TeamOptionInfo("""friendly Fire""", "friendlyFire", """Enables/Disables players inflicting damage on each other when on the same team. (Note: players can still inflict status effects on each other.) Does not affect some non-player entities in a team.""", bool),
    "SEE_FRIENDLY_INVISIBLES": TeamOptionInfo("""see Friendly Invisibles""", "seeFriendlyInvisibles", """Decides whether invisible players is semi-transparent or completely invisible for other players on their team.""", bool),
    ...
    "PREFIX": TeamOptionInfo("""prefix""", "prefix", """Modifies the prefix that displays at the beginning of players' names.""", "JsonDef"),
    "SUFFIX": TeamOptionInfo("""suffix""", "suffix", """Modifies the suffix that displays at the end of players' names.""", "JsonDef"),
}
...
# Patterns
# Derived from https://minecraft.wiki/Banner/Patterns, 2025-01-02T17:56:53-08:00
__pattern_dups = {}
BASE = "base"
...
# Advancements
# Derived from https://minecraft.wiki/Advancement#List_of_advancements, 2025-01-02T17:56:53-08:00
__advancement_dups = {}
...
advancements = {
    ...
    "COUNTRY_LODE_TAKE_ME_HOME": AdvancementInfo("""Country Lode, Take Me Home""", "nether/use_lodestone", """Use a Compass on a Lodestone."""),
...
# Biomes
# Derived from https://minecraft.wiki/Biome/ID, 2025-01-02T17:56:53-08:00
__biome_dups = {}
THE_VOID = "the_void"
...
# Effects
# Derived from https://minecraft.wiki/Effect?so=search#Effect_list, 2025-01-02T17:56:54-08:00
__effect_dups = {}
SPEED = "speed"
SLOWNESS = "slowness"
HASTE = "haste"
...
effects = {
    ...
    "INVISIBILITY": EffectInfo("""Invisibility""", "invisibility", """Grants Invisibility, making the affected entity invisible (but not the item they hold or the armor they wear), and reduces other mobs' detection range for the affected entity, higher levels reduce other mobs' detection range more.""", True),
    ...
    "LUCK": EffectInfo("""Luck""", "luck", """Increases chances of better and more loot, higher levels increase the chances of better and more loot.""", True),
    ...
    "BAD_OMEN": EffectInfo("""Bad Omen""", "bad_omen", """Causes an ominous event upon entering a village or the trial chambers.""", None),
...
# Enchantments
# Derived from https://minecraft.wiki/Enchanting#Summary_of_enchantments, 2025-01-02T17:56:55-08:00
__enchantment_dups = {}
...
enchantments = {
    ...
    "BREACH": EnchantmentInfo("""Breach""", "breach", """Reduces the effectiveness of the armor on the target. The armor's effectiveness is reduced by 15% per level.""", 4),
    ...
    "DENSITY": EnchantmentInfo("""Density""", "density", """Increases the damage dealt per block fallen, increasing by 0.5 per level.""", 5),
    "DEPTH_STRIDER": EnchantmentInfo("""Depth Strider""", "depth_strider", """Increases underwater movement speed.""", 3),
    ...
    "FIRE_PROTECTION": EnchantmentInfo("""Fire Protection""", "fire_protection", """Reduces fire damage and burn time.""", 4),
    "FLAME": EnchantmentInfo("""Flame""", "flame", """Arrows shot are ignited and deal fire damage to the target.""", 1),
    "FORTUNE": EnchantmentInfo("""Fortune""", "fortune", """Increases the amount of block drops.""", 3),
    "FROST_WALKER": EnchantmentInfo("""Frost Walker""", "frost_walker", """Freezes the water under the player's feet, allowing them to walk over water.""", 2),
    "IMPALING": EnchantmentInfo("""Impaling""", "impaling", """Increases damage against aquatic mobs. In Bedrock Edition, increases damage against mobs in water or rain.""", 5),
    ...
    "LUCK_OF_THE_SEA": EnchantmentInfo("""Luck of the Sea""", "luck_of_the_sea", """Increases rate of treasure bites (enchanting books, etc.).""", 3),
    ...
    "PIERCING": EnchantmentInfo("""Piercing""", "piercing", """Arrows pierce entities, allowing for arrows to pierce through stacks of mobs.""", 4),
    ...
    "RIPTIDE": EnchantmentInfo("""Riptide""", "riptide", """Trident launches player with itself when thrown while in water or rain.""", 3),
    ...
    "SOUL_SPEED": EnchantmentInfo("""Soul Speed""", "soul_speed", """Increases movement speed on soul sand and soul soil.""", 3),
    ...
    "THORNS": EnchantmentInfo("""Thorns""", "thorns", """If a mob wearing armor with that enchantment takes damage, the attacker will also take damage.""", 3),
    "UNBREAKING": EnchantmentInfo("""Unbreaking""", "unbreaking", """Reduces durability damage.""", 3),
    "WIND_BURST": EnchantmentInfo("""Wind Burst""", "wind_burst", """Emits a burst of wind (like that of a wind charge) upon executing a mace smash attack on an entity, launching the attacker upward. It can be used to chain smash attacks together, and the strength of the launch increases per level.""", 3),
...
# GameRules
# Derived from https://minecraft.wiki/Game_rule?so=search#List_of_game_rules, 2025-01-02T17:56:55-08:00
__gamerule_dups = {}
...
game_rules = {
    ...
    "FORGIVE_DEAD_PLAYERS": GameRuleInfo("""forgive Dead Players""", "forgiveDeadPlayers", """Makes angered neutral mobs stop being angry when the targeted player dies nearby.""", bool),
    "FREEZE_DAMAGE": GameRuleInfo("""freeze Damage""", "freezeDamage", """Whether the player should take damage when inside powder snow.""", bool),
    "GLOBAL_SOUND_EVENTS": GameRuleInfo("""global Sound Events""", "globalSoundEvents", """Whether certain sound events are heard by all players regardless of location.""", bool),
    ...
    "MAX_ENTITY_CRAMMING": GameRuleInfo("""max Entity Cramming""", "maxEntityCramming", """The maximum number of pushable entities a mob or player can push, before taking 6 entity cramming damage per half-second. Setting to 0 or lower disables the rule. Damage affects Survival-mode or Adventure-mode players, and all mobs but bats. Pushable entities include non-Spectator-mode players, any mob except bats, as well as boats and minecarts.""", int),
...
# ScoreCriteria
# Derived from https://minecraft.wiki/Scoreboard#Criteria, 2025-01-02T17:56:55-08:00
...
# Particles
# Derived from https://minecraft.wiki/Particles_(Java_Edition)#Types_of_particles, 2025-01-02T17:56:56-08:00
__particle_dups = {}
...
WITCH = "witch"
PARTICLE_GROUP = [
    AMBIENT_ENTITY_EFFECT, ANGRY_VILLAGER, ASH, "block", BLOCK_CRUMBLE, "block_marker", BUBBLE, BUBBLE_COLUMN_UP, BUBBLE_POP, CAMPFIRE_COSY_SMOKE, CAMPFIRE_SIGNAL_SMOKE, CHERRY_LEAVES, CLOUD, COMPOSTER, CRIMSON_SPORE, CRIT, CURRENT_DOWN, DAMAGE_INDICATOR, DOLPHIN, DRAGON_BREATH, DRIPPING_DRIPSTONE_LAVA, DRIPPING_DRIPSTONE_WATER, DRIPPING_HONEY, DRIPPING_LAVA, DRIPPING_OBSIDIAN_TEAR, DRIPPING_WATER, DUST, DUST_COLOR_TRANSITION, "dust_pillar", DUST_PLUME, EFFECT, EGG_CRACK, ELDER_GUARDIAN, ELECTRIC_SPARK, ENCHANT, ENCHANTED_HIT, END_ROD, ENTITY_EFFECT, EXPLOSION, EXPLOSION_EMITTER, FALLING_DRIPSTONE_LAVA, FALLING_DRIPSTONE_WATER, "falling_dust", FALLING_HONEY, FALLING_LAVA, FALLING_NECTAR, FALLING_OBSIDIAN_TEAR, FALLING_SPORE_BLOSSOM, FALLING_WATER, FIREWORK, FISHING, "flame", FLASH, GLOW, GLOW_SQUID_INK, GUST, GUST_EMITTER, HAPPY_VILLAGER, HEART, "infested", INSTANT_EFFECT, "item", ITEM_COBWEB, ITEM_SLIME, ITEM_SNOWBALL, LANDING_HONEY, LANDING_LAVA, LANDING_OBSIDIAN_TEAR, LARGE_SMOKE, LAVA, MYCELIUM, NAUTILUS, NOTE, OMINOUS_SPAWNING, PALE_OAK_LEAVES, POOF, PORTAL, "raid_omen", "rain", REVERSE_PORTAL, SCRAPE, SCULK_CHARGE, SCULK_CHARGE_POP, SCULK_SOUL, SHRIEK, SMALL_FLAME, SMALL_GUST, SMOKE, SNEEZE, SNOWFLAKE, SONIC_BOOM, SOUL, SOUL_FIRE_FLAME, SPIT, SPLASH, SPORE_BLOSSOM_AIR, SQUID_INK, SWEEP_ATTACK, TOTEM_OF_UNDYING, TRAIL, "trial_omen", TRIAL_SPAWNER_DETECTION, TRIAL_SPAWNER_DETECTION_OMINOUS, UNDERWATER, VAULT_CONNECTION, VIBRATION, WARPED_SPORE, "wax_off", "wax_on", WHITE_ASH, WHITE_SMOKE, WITCH
]

ParticleInfo = namedtuple("Particle", ['name', 'value', 'desc'])
particles = {
    "AMBIENT_ENTITY_EFFECT": ParticleInfo("""Ambient Entity Effect""", "ambient_entity_effect", """Emitted by entities with effects from a beacon or a conduit."""),
    "ANGRY_VILLAGER": ParticleInfo("""Angry Villager""", "angry_villager", """Produced when hitting villagers or when villagers fail to breed."""),
    ...
    "BLOCK_MARKER": ParticleInfo("""Block Marker""", "block_marker", """Marks the position of barriers, structure voids and light blocks when they are held in the main hand."""),
    ...
    "ENCHANTED_HIT": ParticleInfo("""Enchanted Hit""", "enchanted_hit", """Appears when hitting entities with a sword or an axe enchanted with Sharpness, Bane of Arthropods, or Smite."""),
    ...
    "INSTANT_EFFECT": ParticleInfo("""Instant Effect""", "instant_effect", """Produced when splash potions or lingering potions of Instant Health or Instant Damage break."""),
    ...
    "PALE_OAK_LEAVES": ParticleInfo("""Pale Oak Leaves""", "pale_oak_leaves", """Dropped from pale oak leaves in pale garden biomes."""),
...
# PotterySherds
# Derived from https://minecraft.wiki/Pottery_Sherd, 2025-01-02T17:56:56-08:00
__potterysherd_dups = {}
...
# Discs
# Derived from https://minecraft.wiki/Music_Disc#Discs, 2025-01-02T17:56:56-08:00
__disc_dups = {}
...
DISC_GROUP = [
    THIRTEEN, CAT, BLOCKS, CHIRP, FAR, MALL, MELLOHI, STAL, STRAD, WARD, ELEVEN, WAIT, OTHERSIDE, FIVE, PIGSTEP, RELIC, CREATOR, CREATOR_MUSIC_BOX, PRECIPICE
]
...
# Paintings
# Derived from https://minecraft.wiki/Painting#Canvases, 2025-01-02T17:56:57-08:00
__painting_dups = {}
...
WATER = "Water"
PAINTING_GROUP = [
    KEBAB, AZTEC, ALBAN, AZTEC2, BOMB, PLANT, WASTELAND, MEDITATIVE, WANDERER, GRAHAM, PRAIRIE_RIDE, POOL, COURBET, "sunset_dense", SEA, CREEBET, MATCH, BUST, STAGE, VOID, SKULL_AND_ROSES, "Wither", BAROQUE, HUMBLE, BOUQUET, CAVEBIRD, COTAN, ENDBOSS, FERN, OWLEMONS, SUNFLOWERS, TIDES, BACKYARD, POND, FIGHTERS, CHANGING, FINDING, LOWMIST, PASSAGE, SKELETON, DONKEY_KONG, POINTER, PIGSCENE, BURNING_SKULL, ORB, UNPACKED, EARTH, WIND, FIRE, WATER
]
...
paintings = {
    ...
    "WITHER": PaintingInfo("""wither""", "Wither", """The creation of the wither. This is the first painting in Minecraft that is not based of a real painting. (See the trivia section for more info.).""", "Mojang(Jens Bergensten)", (2, 2)),
