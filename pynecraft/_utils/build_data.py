...
def _build_font_data():
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        f.write('\n')
        f.write('CHAR_WIDTHS: dict[str, tuple[float, float]] = {\n')
        for char, info in sorted(chars.items(), key=lambda x: ord(x[0])):
            normal = info['width']
            bold = info['width'] + info['bold_offset']
            f.write(f'    {repr(char)}: ({normal}, {bold}),\n')
        f.write('}\n')
    print(f'Written {out_path} ({len(chars)} characters)')
